...
HHH_TT_X(bool scaled=true, bool log=true, float min=0.1, float max=-1., std::string inputfile="root/$HISTFILE", const char* directory="tauTau_$CATEGORY")
{
...
//  TH1F* EWK2   = refill((TH1F*)input->Get(TString::Format("%s/ZJ"      , directory)), "ZJ" ); InitHist(EWK2 , "", "", TColor::GetColor(222,90,106), 1001);
  TH1F* Zll   = refill((TH1F*)input->Get(TString::Format("%s/ZLL"      , directory)), "ZL" ); InitHist(Zll , "", "", TColor::GetColor(222,90,106), 1001);
...
*/
 ...
 // unscaled[1]+= EWK2 ->Integral();
  ...
  unscaled[4] = Zll ->Integral();
#ifdef MSSM
  unscaled[5] = ggHTohhTo2Tau2B  ->Integral();
/*
...
*/
  ...
  scales[4] = new TH1F("scales-Zll"  , "", 8, 0, 8);
  scales[4]->SetBinContent(5, unscaled[4]>0 ? (Ztt  ->Integral()/unscaled[4]-1.) : 0.);
#ifdef MSSM
  scales[5] = new TH1F("scales-ggHTohhTo2Tau2B"  , "", 8, 0, 8);
  scales[5]->SetBinContent(6, unscaled[5]>0 ? (ggHTohhTo2Tau2B  ->Integral()/unscaled[5]-1.) : 0.);
/*
...
*/
  ...
  */
  ...
  InitHist(scales[4],"","",TColor::GetColor(155,152,204),1001);
...
*/
  ...
  scales[0]->GetXaxis()->SetBinLabel(5, "#bf{Zll}"  );
#ifdef MSSM
  scales[0]->GetXaxis()->SetBinLabel(6, "#bf{ggHTohhTo2tau2B}"  );
/*
